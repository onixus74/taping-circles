...
namespace Grow.Dlc
{
	...
	public class GrowDLC {
		...
		private static extern int growDlc_initialize();
		[DllImport ("__Internal")]
		private static extern int growDlc_checkSyncedPackagesStatus();
		[DllImport ("__Internal")]
		private static extern int growDlc_checkPackageStatus(string packageId);
		[DllImport ("__Internal")]
		private static extern int growDlc_startSync(string packageId, out bool outResult);
		[DllImport ("__Internal")]
		private static extern int growDlc_getFilePath(string packageId, string fileName, out IntPtr filePathP);
		[DllImport ("__Internal")]
		private static extern int growDlc_getFilesPathsInPackage(string packageId, out IntPtr json);
		#endif
		...
		public static void Initialize() {
			...
			using(AndroidJavaClass jniGrowDLCClass = new AndroidJavaClass("com.soomla.dlc.GrowDLC")) {

				AndroidJavaObject jniGrowDLCInstance = jniGrowDLCClass.CallStatic<AndroidJavaObject>("getInstance");
				jniGrowDLCInstance.Call("initialize");
			}
		...
		public static void CheckSyncedPackagesStatus() {
			...
			using(AndroidJavaClass jniGrowDLCClass = new AndroidJavaClass("com.soomla.dlc.GrowDLC")) {
				...
				AndroidJavaObject jniGrowDLCInstance = jniGrowDLCClass.CallStatic<AndroidJavaObject>("getInstance");
				jniGrowDLCInstance.Call("checkSyncedPackagesStatus");
			}
			AndroidJNI.PopLocalFrame(IntPtr.Zero);
			#elif UNITY_IOS && !UNITY_EDITOR
			growDlc_checkSyncedPackagesStatus();
			#endif
		}
		...
		public static void CheckPackageStatus(string packageId) {
			...
			using(AndroidJavaClass jniGrowDLCClass = new AndroidJavaClass("com.soomla.dlc.GrowDLC")) {
				AndroidJavaObject jniGrowDLCInstance = jniGrowDLCClass.CallStatic<AndroidJavaObject>("getInstance");
				jniGrowDLCInstance.Call("checkPackageStatus", packageId);
			}
			AndroidJNI.PopLocalFrame(IntPtr.Zero);
			#elif UNITY_IOS && !UNITY_EDITOR
			growDlc_checkPackageStatus(packageId);
			#endif
		}
		...
		public static bool StartSync(string packageId) {
			...
			using(AndroidJavaClass jniGrowDLCClass = new AndroidJavaClass("com.soomla.dlc.GrowDLC")) {
				...
				AndroidJavaObject jniGrowDLCInstance = jniGrowDLCClass.CallStatic<AndroidJavaObject>("getInstance");
				result = jniGrowDLCInstance.Call<bool>("startSync", packageId);
			}
			AndroidJNI.PopLocalFrame(IntPtr.Zero);
			#elif UNITY_IOS && !UNITY_EDITOR
			growDlc_startSync(packageId, out result);
			#endif
			return result;
		}
		...
		public static string GetFilePath(string packageId, string fileName) {
			...
			using(AndroidJavaClass jniGrowDLCClass = new AndroidJavaClass("com.soomla.dlc.GrowDLC")) {
				...
				AndroidJavaObject jniGrowDLCInstance = jniGrowDLCClass.CallStatic<AndroidJavaObject>("getInstance");
				result = jniGrowDLCInstance.Call<string>("getPathToFile", packageId, fileName);
			}
			AndroidJNI.PopLocalFrame(IntPtr.Zero);
			#elif UNITY_IOS && !UNITY_EDITOR
			IntPtr p = IntPtr.Zero;
			growDlc_getFilePath(packageId, fileName, out p);
		...
		public static IList<string> GetFilesPathsInPackage(string packageId) {
			...
			using(AndroidJavaClass jniGrowDLCClass = new AndroidJavaClass("com.soomla.highway.unity.GrowDLC")) {

				result = jniGrowDLCClass.CallStatic<string>("unityGetFilesPathsInPackage", packageId);
			}
			AndroidJNI.PopLocalFrame(IntPtr.Zero);
			#elif UNITY_IOS && !UNITY_EDITOR
			IntPtr p = IntPtr.Zero;
			growDlc_getFilesPathsInPackage(packageId, out p);
		...
		private const string TAG = "SOOMLA GrowDLC";
	}
}
